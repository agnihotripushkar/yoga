...
class ClassesService(
    ...
    fun getClasses(searchRequest: ClassSearchRequest, userId: Long): ClassListResponse {
        ...
        val userFavorites =
                classFavoriteRepository.findFavoriteClassesByUser(user).map { it.id }.toSet()

        val classSummaries =
                classesPage.content.map { yogaClass ->
        ...
        return ClassListResponse(classes = classSummaries, pagination = pagination)
    }
    ...
    fun searchClasses(searchRequest: ClassSearchRequest, userId: Long): ClassListResponse {
        logger.debug(
                "Searching classes with query: '{}' for userId: {}",
                searchRequest.query,
                userId
        )
    ...
    fun addToFavorites(classId: Long, userId: Long): FavoriteResponse {
        ...
        val favorite = ClassFavorite(user = user, yogaClass = yogaClass)
    ...
    fun removeFromFavorites(classId: Long, userId: Long): FavoriteResponse {
        ...
        if (deletedCount > 0) {
            logger.info(
                    "Successfully removed class {} from favorites for userId: {}",
                    classId,
                    userId
            )
            return FavoriteResponse(
    ...
    private fun createPageable(searchRequest: ClassSearchRequest): Pageable {
        val sortDirection =
                if (searchRequest.sortDirection.lowercase() == "desc") {
                    Sort.Direction.DESC
                } else {
                    Sort.Direction.ASC
                }
        ...
        val sortBy =
                when (searchRequest.sortBy.lowercase()) {
                    "title" -> "title"
